...
Версия **v2.9.3** обновляет Fizeau до актуальной `libryazhahand`. Оверлей получает стиль **Switch 2** по умолчанию, если он не отключён в глобальной конфигурации Ryazhahand, а меню разделено на понятные блоки: основное, режим дня и ночи, цвет, изображение и интерфейс.

Изменённые слайдеры и параметры больше не теряются при выходе кнопкой **B**: Fizeau применяет ожидающие изменения перед возвратом или закрытием экрана. Смена языка теперь заменяет текущий экран, а не добавляет его в стек, поэтому **B** всегда возвращает на ожидаемый предыдущий уровень.

Для новых установок коррекция **выключена по умолчанию**. Включите её вручную в разделе **«Основное» → «Коррекция»** только после проверки выбранного профиля и параметров изображения.
...
### Интерфейс и сборка
- Стиль **Switch 2** из актуальной libryazhahand включён по умолчанию; пользователь может изменить глобальную настройку Ryazhahand.
- Кнопка **B** сохраняет уже изменённые параметры и только затем возвращает назад или закрывает оверлей.
- Автосборка через GitHub Actions при каждом push.
- Release `v2.9.3` создаётся из единой переменной версии `FZ_VERSION`.
- Оверлей собирается на **libryazhahand** — библиотеке интерфейса экосистемы Ряженка.
...
4. Открой Fizeau через Ryazhahand или другой Tesla-совместимый лаунчер.
5. Меняйте параметры слайдерами; кнопка **B** сохранит уже внесённые изменения и вернёт назад.
